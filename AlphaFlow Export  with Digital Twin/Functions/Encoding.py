...
#short term memory for classifier and regr. not a class just a bad name
def ClassShortTermMemoryX1(X, MemorySteps, BlockEncoding):
    ...
    for XStep in range(X.shape[0]):
        ...
        for MemoryStep in range(MemorySteps):
            ...
            # print(XStep)
            # print(MemoryStep)
            #if any step in STM or current step == 0 or Xstep = 0 (redundant? but afraid) or a baby with not enough memory steps
